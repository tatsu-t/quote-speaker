...
// Configuration
// VOICEVOX_URL in VPS context refers to the Fallback URL (Tunnel)
const VOICEVOX_LOCAL_URL = process.env.VOICEVOX_URL || 'http://host.docker.internal:5000';
...
async function generateVoicevoxAudio(text, speakerId = 1) {
    ...
    if (pointsData) {
        ...
        if (points < 1000) { // Threshold for low points
            console.warn("API Points Low! Switching to Remote Processing (Tunnel).");
            useLocal = true;
        }
    }
    ...
    if (!useLocal) {
        try {
            ...
            console.warn('External API Failed (Error or Points run out). Switching to Remote Processing Fallback...', externalError.message);
            useLocal = true;
        }
    }

    // 3. Local Engine Fallback (Via Tunnel)
    if (useLocal) {
        try {
            // Step 1: Query
            ...
            console.error('Remote/Local Fallback Failed:', localError.message);
            throw new Error("TTS Generation Failed on both External and Remote engines.");
        }
    }
}
...
client.once('ready', async () => {
    console.log('Ready!');
    console.log(`Fallback Voicevox URL set to: ${VOICEVOX_LOCAL_URL}`);
